...
def render():
    ...
    with get_session() as session:
        ...
        if sites:
            ...
            for site in sites:
                with st.expander(f"{site.site_name}", expanded=True):
                    ...
                    with col2:
                        ...
                        # Show associated restaurants count
                        rest_count = len(site.restaurants)
                        st.metric("Restaurants", rest_count)
                        
                        # Show associated jobs count
                        jobs_count = len(site.jobs)
